...
class PixelDecoder {
    ...
    constructor (code_size, pixel_data, static_areas) {
        ...
        do {
            ...
            [curr_x, curr_y, end_reached] = this.#next_data_pixel_pos(static_areas, curr_x, curr_y);
        } while (!end_reached);
    }
    ...
    #next_data_pixel_pos (static_areas, x, y) {
        // simply skip over positions that are inside an area:
        var x, y, end_reached;
        do {
            [x, y, end_reached] = this.#next_position_in_grid(x, y);
            if (end_reached) {
