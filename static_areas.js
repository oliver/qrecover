...
function add_static_areas () {
    ...
    add_area("pos_ul", RegionList.from_nested_arrays([[0, 0, 7, 7]]), [0, 255, 0], function (area) {
    ...
    add_area("pos_ur", RegionList.from_nested_arrays([[code_size-7, 0, 7, 7]]), [0, 255, 0], static_areas.get("pos_ul").check_function);
    add_area("pos_bl", RegionList.from_nested_arrays([[0, code_size-7, 7, 7]]), [0, 255, 0], static_areas.get("pos_ul").check_function);

    // one orientation marker (bottom right):
    add_area("orient", RegionList.from_nested_arrays([[16, 16, 5, 5]]), [0, 255, 0], function (area) {
        expect_pixels(area.regions,
            "xxxxx" +
            "x...x" +
            "x.x.x" +
    ...
    add_area("spacing_ul_1", RegionList.from_nested_arrays([[0, 7, 8, 1], [7, 0, 1, 7]]), [192, 255, 128], function (area) {
        expect_pixels(area.regions, "........" + ".......");
        return {"valid": true};
    });
    add_area("spacing_ur_1", RegionList.from_nested_arrays([[code_size-8, 0, 1, 7], [code_size-8, 7, 8, 1]]), [192, 255, 128], static_areas.get("spacing_ul_1").check_function);
    add_area("spacing_bl_1", RegionList.from_nested_arrays([[0, code_size-8, 8, 1], [7, code_size-7, 1, 7]]), [192, 255, 128], static_areas.get("spacing_ul_1").check_function);

    // two timing strips:
    add_area("timing_top", RegionList.from_nested_arrays([[8, 6, 9, 1]]), [0, 0, 255], function (area) {
        expect_pixels(area.regions, "x.x.x.x.x");
        return {"valid": true};
    });
    add_area("timing_left", RegionList.from_nested_arrays([[6, 8, 1, 9]]), [0, 0, 255], static_areas.get("timing_top").check_function);

    // error correction level (redundantly):
    add_area("format_ec_1", RegionList.from_nested_arrays([[0, 8, 2, 1]]), [255, 128, 0], function (area) {
    ...
    add_area("format_ec_2", RegionList.from_nested_arrays([[8, code_size-1, 1, 1], [8, code_size-2, 1, 1]]), [255, 128, 0], function (area) {
    ...
    add_area("format_mask_1", RegionList.from_nested_arrays([[2, 8, 3, 1]]), [128, 255, 0], function (area) {
    ...
    add_area("format_mask_2", RegionList.from_nested_arrays([[8, code_size-3, 1, 1], [8, code_size-4, 1, 1], [8, code_size-5, 1, 1]]), [128, 255, 0], function (area) {
        const bits = pixel_data.get_pixels_from_regions_as_bools(area.regions);
    ...
    add_area("format_ec_data_1", RegionList.from_nested_arrays([
    ...
    add_area("format_ec_data_2", RegionList.from_nested_arrays([[8, code_size-6, 1, 1], [8, code_size-7, 1, 1], [code_size-8, 8, 8, 1]]), [192, 128, 128], function(area) {
    ...
    add_area("dark_module", RegionList.from_nested_arrays([[8, code_size-8, 1, 1]]), [128, 128, 192], function(area) {
