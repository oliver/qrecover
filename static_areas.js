...
function add_static_areas (decoder) {
    ...
    // two timing strips:
    new_static_areas.add_area(new Area("timing_top", RegionList.from_nested_arrays([[8, 6, 9, 1]]), [0, 0, 255], function (area) {
    ...
    new_static_areas.add_area(new Area("format_ec_1", RegionList.from_nested_arrays([[0, 8, 2, 1]]), [255, 128, 0], function (area) {
    ...
    new_static_areas.add_area(new Area("format_ec_2", RegionList.from_nested_arrays([[8, code_size-1, 1, 1], [8, code_size-2, 1, 1]]), [255, 128, 0], function (area) {
        const bits = decoder.pixel_data.get_pixels_from_regions_as_bools(area.regions);
        const ec_level = bits_to_int(bits) ^ 0b10;

        const ec_1_value = new_static_areas.get("format_ec_1").value_details.value;
        var valid = true;
    ...
    new_static_areas.add_area(new Area("format_mask_1", RegionList.from_nested_arrays([[2, 8, 3, 1]]), [128, 255, 0], function (area) {
    ...
    new_static_areas.add_area(new Area("format_mask_2", RegionList.from_nested_arrays([[8, code_size-3, 1, 1], [8, code_size-4, 1, 1], [8, code_size-5, 1, 1]]), [128, 255, 0], function (area) {
        const bits = decoder.pixel_data.get_pixels_from_regions_as_bools(area.regions);
        const mask_value = bits_to_int(bits) ^ 0b101;

        const mask_1_value = new_static_areas.get("format_mask_1").value_details.value;
        var valid = true;
    ...
    new_static_areas.add_area(new Area("format_ec_data_1", RegionList.from_nested_arrays([
    ...
    new_static_areas.add_area(new Area("format_ec_data_2", RegionList.from_nested_arrays([[8, code_size-6, 1, 1], [8, code_size-7, 1, 1], [code_size-8, 8, 8, 1]]), [192, 128, 128], function(area) {
    ...
    new_static_areas.add_area(new Area("dark_module", RegionList.from_nested_arrays([[8, code_size-8, 1, 1]]), [128, 128, 192], function(area) {
