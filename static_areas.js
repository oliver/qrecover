...
function expect_pixels (regions, expected_values) {
    const bool_array = pixel_data.get_pixels_from_regions_as_bools(RegionList.from_raw_objects(regions));
...
function add_static_areas () {
    ...
    add_area("format_ec_1", [[0, 8, 2, 1]], [255, 128, 0], function (area) {
        const [bit0, bit1] = pixel_data.get_pixels_from_regions_as_bools(RegionList.from_raw_objects(area.regions));
    ...
    add_area("format_ec_2", [[8, code_size-1, 1, 1], [8, code_size-2, 1, 1]], [255, 128, 0], function (area) {
        const bits = pixel_data.get_pixels_from_regions_as_bools(RegionList.from_raw_objects(area.regions));
    ...
    add_area("format_mask_1", [[2, 8, 3, 1]], [128, 255, 0], function (area) {
        const [bit0, bit1, bit2] = pixel_data.get_pixels_from_regions_as_bools(RegionList.from_raw_objects(area.regions));
    ...
    function do_complete_format_ec_check (area, format_ec_area_id, format_mask_area_id) {
        const value = bits_to_int(get_masked_pixels().get_pixels_from_regions_as_bools(RegionList.from_raw_objects(area.regions)));
